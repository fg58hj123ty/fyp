...
class DoorSystem:
    ...
    def handle_verification(self):
        ...
        try:
            clear_screen()
            print_styled_banner("Starting Voice Verification", 'info')
            ...
            # Verify user
            verified_user = self.verify_user(embedding)
            time.sleep(3)
            if verified_user:
                clear_screen()
    ...
    def verify_user(self, new_embedding):
        with self.app.app_context():  
            try:
                ...
                if not voice_profiles:
                    print_styled_banner("No voice profiles found in database", 'error')
                    return None
...
class AudioCapture:
    ...
    def start_recording(self):
        """Start recording audio from microphone and return WAV file buffer"""
        clear_screen()
        print_styled_banner("Available Input Devices:", 'info')
        self.print_devices()
        
        try:
            # Reset frames list for new recording
            self.frames = []
            
            # Close any existing streams properly
            ...
            # Create a new stream for this recording
            ...
            # Clear initial buffer without counting it in the recording time
            for _ in range(3):
            ...
            for i in range(3, 0, -1):
                ...
                stream.read(self.chunk_size, exception_on_overflow=False)
                time.sleep(1.0)  # Full second for countdown
            
            clear_screen()
            print_styled_banner("Recording Started", 'info')
            
            # Calculate number of chunks needed for exact duration
            chunks_needed = int((self.record_seconds * self.sample_rate) / self.chunk_size)
            chunks_recorded = 0
            start_time = time.time()

            # Record exact number of chunks
            while chunks_recorded < chunks_needed:
                ...
                print_recording_progress(elapsed_time, remaining_time, self.record_seconds)
            ...
            # Ensure stream is properly closed even if an error occurs
            if 'stream' in locals():
                try:
                    stream.stop_stream()
                    stream.close()
...
def print_recording_progress(elapsed_time, remaining_time, total_time):
    """Print recording progress bar with time"""
    width = 40
    filled = min(width, int(width * elapsed_time / total_time))
    bar = '█' * filled + '▒' * (width - filled)
    print(f'\rRecording: [{bar}] {min(elapsed_time, total_time):.1f}s / {total_time:.1f}s', end='', flush=True)
